...
const ScrollStack: React.FC<ScrollStackProps> = ({
  ...
  const measureStaticOffsets = useCallback((cards: HTMLElement[]) => {
    ...
    // Measure untransformed elements
    ...
    // Instantly restore styles in the same synchronous frame
    cards.forEach((card, idx) => {
      if (cachedStyles[idx]) {
        card.style.transform = cachedStyles[idx].transform;
        card.style.filter = cachedStyles[idx].filter;
      }
    });
  }, [useWindowScroll]);
  ...
  const setupLenis = useCallback(() => {
    if (useWindowScroll) {
      // Avoid instantiating a duplicate window Lenis instance. 
      // Instead, seamlessly hook into the existing root Lenis scroll event.
  ...
  useEffect(() => {
    ...
    // Measure static card and page tops prior to animating them
    measureStaticOffsets(cards);
